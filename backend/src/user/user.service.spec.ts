...
describe('UserResolver & UserService', () => {
  ...
  describe('UserService', () => {
    ...
    it('login returns LoginResponse on success', async () => {
      ...
      jwtService.sign.mockImplementation(() => 'signed-token');
    ...
    // FIX: handle bcrypt.compare errors gracefully and throw UnauthorizedException with 'Invalid credentials' message
    ...
    it('signup throws error if bcrypt.hash fails', async () => {
      jest.spyOn(bcrypt, 'hash').mockImplementation(() => Promise.reject(new Error('Hash error')));
